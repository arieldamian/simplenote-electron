...
import { replaceTags } from '../state/tags/actions';
import {
	getSelectedTag,
	getTags,
	...
	handlers: {
		...
		reorderTags: {
			creator( { tagBucket, tags } ) {
				return () => tags
					.map( ( tag, index ) => ( { ...tag, data: { ...tag.data, index } } ) )
					.forEach( ( { id, data } ) => tagBucket.update( id, data ) );
			}
		},
			...
			creator( { noteBucket } ) {
				return ( dispatch, getState ) => {
					...
					noteBucket.add( {
						...
						shareURL: '',
						publishURL: '',
						tags: castArray( get( selectedTag, 'data.name', [] ) )
					}, ( e, note ) => {
		...
		updateNoteTags: {
			creator( { noteBucket, tagBucket, note, tags } ) {
				return ( dispatch, getState ) => {
					...
					const existingTags = getTags( getState() );
					...
					let currentTagNames = existingTags.map( tag => tag.data.name );
